...
function Salidas() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const updatedSalidas = filteredSalidas.filter(
        (salida) => salida.id !== id
      );
        ...
        <tbody>
          {filteredSalidas.map((salida) => (
            <tr key={salida.id}>
              ...
              <td>
                {salida.productos_con_medida &&
                salida.productos_con_medida.length > 0 ? (
              ...
              <td>
                ...
                salida.productos_con_medida.length > 0 ? (
                  salida.productos_con_medida.map((producto, index) => (
                    <div key={index}>{producto.pivot.cantidad}</div>
                  ))
                ) : (
              ...
              <td>
                ...
                salida.productos_con_medida.length > 0 ? (
                  salida.productos_con_medida.map((producto, index) => (
                    <div key={index}>{producto.medida_nombre}</div>
                  ))
                ) : (

...
function StockGeneral() {
    ...
    const fetchStockData = async () => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/api/stock-general"
        );
        setStockData(response.data);
        calculateTotalStock(response.data);

        const productosSet = new Set(
          response.data.map((item) => item.producto.nombre)
        );
        setProductos(Array.from(productosSet));

        const medidasSet = new Set(
          response.data.map((item) => item.medida.nombreMedida)
        );
  ...
  const calculateTotalStock = (data) => {
    const total = data.reduce(
      (acc, item) => acc + parseInt(item.stock_total),
      0
    );
    setTotalStock(total);
  };
  ...
  const handleAddProducto = (producto, medida, cantidad) => {
    ...
    setSelectedProductos((prev) => {
      const exists = prev.find(
        (item) =>
          item.id_producto === newProduct.id_producto &&
          item.id_medida === newProduct.id_medida
      );
      if (exists) {
  ...
  const handleRegisterSalida = async () => {
    try {
      ...
      const updatedStockResponse = await axios.get(
        "http://127.0.0.1:8000/api/stock-general"
      );
  ...
  const filteredStockData = stockData.filter((item) => {
    const productMatch = selectedProducto
      ? item.producto.nombre === selectedProducto
      : true;
    const medidaMatch = selectedMedida
      ? item.medida.nombreMedida === selectedMedida
      : true;
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="mb-3">
        <label htmlFor="productoSeleccionado" className="form-label">
      ...
      <div className="mb-3">
        <label htmlFor="medidaSeleccionada" className="form-label">
          Seleccionar Medida
        </label>
        <select
      ...
      <button
        ...
        onClick={() => {
          const productoSeleccionado = stockData.find(
            (item) => item.producto.nombre === selectedProducto
          );
          const medidaSeleccionada = stockData.find(
            (item) => item.medida.nombreMedida === selectedMedida
          );
      ...
      <button className="btn btn-primary" onClick={handleRegisterSalida}>
        Registrar Salida
      </button>
          ...
          <tbody>
            {selectedProductos.map((item, index) => (
              <tr key={index}>
                <td>
                  {
                    stockData.find((s) => s.producto.id === item.id_producto)
                      ?.producto.nombre
                  }
                </td>
                <td>
                  {
                    stockData.find((s) => s.medida.id === item.id_medida)
                      ?.medida.nombreMedida
                  }
                </td>
                <td>{item.cantidad}</td>

...
function StockTerminado() {
  ...
  const fetchStockData = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/productos-medidas"
      );
  ...
  const handleAddStock = async (e) => {
    ...
    try {
      const medidaSeleccionada = medidas.find(
        (medida) => medida.id === parseInt(newStock.id_medida)
      );
      ...
      const cantidadBolsones =
        parseInt(medidaSeleccionada.cantidad_bolsones) || 1;
  ...
  const filteredStockData = stockData.filter((item) => {
    const matchProduct = selectedProductId
      ? item.id_producto === selectedProductId
      : true;
    const matchMeasure = selectedMeasureId
      ? item.id_medida === selectedMeasureId
      : true;
  ...
  const totalCantidadUnitaria = filteredStockData.reduce(
    (total, item) => total + parseInt(item.cantidad_unitaria),
    0
  );
  ...
  const formattedData = filteredStockData.map((item) => ({
    ...
    ResultadoBolsas: Math.floor(
      parseInt(item.cantidad_unitaria) /
        (parseInt(item.medida.cantidad_bolsas) || 1)
    ),
    SobranteBolsas:
      parseInt(item.cantidad_unitaria) %
      (parseInt(item.medida.cantidad_bolsas) || 1),
    CantidadBolsones: item.medida ? item.medida.cantidad_bolsones : "N/A",
    ResultadoBolsones: Math.floor(
      parseInt(item.cantidad_unitaria) /
        (parseInt(item.medida.cantidad_bolsones) || 1)
    ),
    SobranteBolsones:
      parseInt(item.cantidad_unitaria) %
      (parseInt(item.medida.cantidad_bolsones) || 1),
  ...
  return (
    <div className="container mt-5">
      ...
      {/* Botón para exportar */}
      <button
        onClick={() => exportToExcel(formattedData)}
          ...
          <table className="table table-bordered w-100">
            ...
            <tbody>
              {filteredStockData.map((item) => {
                ...
                const resultadoBolsas = Math.floor(
                  cantidadUnit / cantidadBolsas
                );
                const sobranteBolsas = cantidadUnit % cantidadBolsas;
                const resultadoBolsones = Math.floor(
                  cantidadUnit / cantidadBolsones
                );
      ...
      <div className="card mt-4">
        ...
        <div className="card-body">
          <form onSubmit={handleAddStock}>
            ...
            <div className="mb-3">
              <label htmlFor="cantidad_unitaria" className="form-label">
                Cantidad Unitaria
              </label>
              <input
                type="number"
